/*
    ...
    Given:-
    Implement insertionSort. Given an array, both algorithms will sort the values in the array. The functions take 2 parameters: an array and an optional 
    comparator function. The comparator function is a callback that will take two values from the array to be compared. The function returns a negative
    value if the first value is less than the second, a positive value if the first value is greater than the second, and 0 if both values are equal. 
...
function insertionSort(arr){
    ...
    for(var i=1; i<arr.length; i++){
        ...
        for(var j=i-1; j>=0 && (arr[j]>currentVal || oldestToYoungest(arr[j], currentVal)); j--){
            arr[j+1] = arr[j];
        }

...
-- If (As we got the vertex1 as the existed key (consisting vertex2) in adjacencyList this.adjacencyList[vertex1])
    We're going to assign a new array to that key, that array would be a filtered array, consisting only vertex1 and vertex2 will be removed to this array.

-- If (As we got the vertex2 as the existed key (consisting vertex2) in adjacencyList this.adjacencyList[vertex2])
    We're going to assign a new array to that key, that array would be a filtered array, consisting only vertex2 and vertex1 will be removed to this array.
*/
// class Graph {
//     constructor() {
//         this.adjacencyList = {};
...
//         if(this.adjacencyList[vertex1])
//             //Return a Filter Array, by filterring all the values that is not equal to vertex2 (Means returning array consisting only vertex2)
//             this.adjacencyList[vertex1] = this.adjacencyList[vertex1].filter(element => element!==vertex2);

//         if(this.adjacencyList[vertex2])
//             //Return a Filter Array, by filterring all the values that is not equal to vertex1 (Means returning array consisting only vertex1)            
...
/* Approach:
-- For removing a Vertex from the Graph, Firsly, we remove all its adjacentVertex (Means, Vertices connected with its Edges), then completely remove the required Vertex.

-- To Removing a Vertex from the Graph, we're going to create a function called 'removeVertex(vertex)' accepting the require 'vertex' need to remove.
...
//                     //adjacentVertex are the vertices that have connections with 'Hong Kong' vertex
...
-- Declared an empty 'result' array, which is going to be the end result after Completion of Depth First Traversal across the Graph.
   Declared an empty 'visited' ojbect, which is going to store the visited vertex by setting the visited vertex as true.
...
//       // Here, current context of 'this' pointing to dfs(), where, this.adjacencyList is not defined. That's we defined it in adjacencyList. 
...
-- Create a 'stack' with an empty array, which is going to keep track of vertices & ADD the Starting vertex to the stack;
   Declared an empty 'result' array or say list, to store the end result, to be return at the very end.
   Declared an empty 'visited' ojbect, to store the visited vertex by assigning the visited vertex as true.
   Just initialize the 'currentVertex';
...
/* Visualiztion:-
     ...
     2 |\    /| 2
      ...
      3 \ \/ / 3 
           D    
BFS: [‘A’, ‘B’, ‘E’, ‘C’, ‘D’, ‘F’] (Alphabetical Order)
                Or
     ['A', 'E', 'B', 'D', 'F', 'C']
...
-- Create a 'queue' with an empty array, which is going to keep track of vertices & ADD the Starting vertex to the queue;
   Declared an empty 'result' array or say list, to store the end result, to be return at the very end.
   Declared an empty 'visited' object, to store the visited vertex by assigning the visited vertex as true.
   Just initialize the 'currentVertex';

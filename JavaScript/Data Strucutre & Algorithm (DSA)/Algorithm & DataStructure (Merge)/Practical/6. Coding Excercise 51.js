/*
    ...
    choosing the pivot. We'll make things simple to start, and will choose the pivot as the first element. This is
    not an ideal choice, but it makes the algorithm easier to understand for now.
    ...
    -- Next, recursively call quicksort again with the left and right halves from the pivot until the array is
    sorted.
    
    It's easiest to implement Quick Sort with the aid of your pivot  helper from the earlier exercise. This
    function is responsible for taking an array, setting the pivot value, and mutating the array so that all values
...
*/ 
    ...
    var kitties = ["LilBub", "Heathcliff", "Blue", "Grumpy"];
...
function startComparator(a, b){
    if(a.length && a>b) return true;
    if(a.age && a.age<b.age) return true;
    else if(!a.length && !a.age && a>b) return true;
    else return false;
